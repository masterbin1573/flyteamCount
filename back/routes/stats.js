  ...
  try {
    ...
    ctx.body = {
      ...
      data: {
        site_pv: site.pv || 0,
        site_uv: site.uv || 0,
        page_pv: site.todayPv || 0
      }
    };

  } catch (error) {
    console.error('获取不蒜子统计数据失败:', error);
    ctx.status = 500;
    ctx.body = { 
      success: false, 
      message: '获取统计数据失败',
 ...
 */
router.get('/dashboard', authenticateToken, async (ctx) => {
  try {
    const userDomain = ctx.user.domain;
    
    // 查找用户的网站
    const site = await Site.findOne({ domain: userDomain });
    if (!site) {
      ctx.body = {
        success: true,
        message: '暂无统计数据',
        data: {
          site: { domain: userDomain, pv: 0, uv: 0, todayPv: 0, todayUv: 0 },
          dailyStats: [],
          browserStats: [],
          osStats: [],
          geoStats: []
    ...
    // 地理位置统计
    const geoStats = await Visit.aggregate([
      { $match: { s: site._id } },
      {
        $group: {
          _id: '$g',
